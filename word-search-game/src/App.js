...
function WordSearch() {
  ...
  const generateGrid = () => {
    ...
    words.forEach((word) => {
      const direction = Math.floor(Math.random() * 8); // 0-7: horizontal, vertical, diagonal, and their reverse directions
      let startX, startY;
      switch (direction) {
        case 0: // horizontal
    ...
    for (let i = 0; i < directions.length; i++) {
      ...
      if (
        ...
        for (let j = startIndex; j <= endIndex; j++) {
          newRow = row + direction.rowOffset * (j - startIndex);
          newCol = col + direction.colOffset * (j - startIndex);
          if (grid[newRow][newCol] !== words[wordIndex][j]) {
            found = false;
            break;
          }
        }
